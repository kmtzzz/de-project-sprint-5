...
class FctProductOriginRepository:
    ...
    def list_facts(self, id_threshold: int, limit: int) -> List[FctProductSalesObj]:
            ...
            cur.execute(
                """
                     select dp.id as product_id,
                        ...
                        from (
                                select object_id as order_object_id,
                     ...
                     WHERE dord.id > %(threshold)s --Load only new data
                    ORDER BY dord.id ASC --Sort by id is mandatory as ID is used as cursor
...
class FctProductDestRepository:
    ...
    def insert_fact(self, conn: Connection, fct_product: FctProductSalesObj) -> None:
        with conn.cursor() as cur:
            cur.execute(
                ...
                """,
                {
                    "product_id": fct_product.product_id,
                    "order_id": fct_product.order_id,
                    "count": fct_product.count,
                    "price": fct_product.price,
                    "total_sum": fct_product.total_sum,
                    "bonus_payment": fct_product.bonus_payment,
                    "bonus_grant": fct_product.bonus_grant
                },
            )
...
class FctProductSalesLoader:
    ...
    BATCH_LIMIT = 10000  # Batch size, 25 is to show incremental load is applicable
    ...
    def load_fct_product_sales(self):
        ...
        with self.pg_dds.connection() as conn:
            ...
            self.log.info(f"Found {len(load_queue)} sales to load to fct_product_sales.")
            if not load_queue:
            ...
            for sale in load_queue:
                self.dds.insert_fact(conn, sale)
